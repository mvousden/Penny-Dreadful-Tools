...
def read_rotation_files() -> Tuple[int, int, List[Card]]:
    runs_str = redis.get_str('magic:rotation:summary:runs')
    runs_percent_str = redis.get_str('magic:rotation:summary:runs_percent')
    cards = redis.get_list('magic:rotation:summary:cards')
...
def rotation_redis_store() -> Tuple[int, int, List[Card]]:
    ...
    # expire after one hour, as the info is out of date by then.
    redis.store('magic:rotation:summary:runs', runs, ex=3600)
    redis.store('magic:rotation:summary:runs_percent', runs_percent, ex=3600)
    redis.store('magic:rotation:summary:cards', cards, ex=3600)
    if 'Undecided' in card_names_by_status:
        redis.sadd('magic:rotation:summary:undecided', *card_names_by_status['Undecided'], ex=3600)
    if 'Legal' in card_names_by_status:
        redis.sadd('magic:rotation:summary:legal', *card_names_by_status['Legal'], ex=3600)
    if 'Not Legal' in card_names_by_status:
        redis.sadd('magic:rotation:summary:notlegal', *card_names_by_status['Not Legal'], ex=3600)
    return (runs, runs_percent, cards)

def get_file_contents(file: str) -> List[str]:
    key = f'magic:rotation:file:{file}'
    contents = redis.get_list(key)
    if contents is not None:
        return contents
    with open(file) as f:
        contents = f.readlines()
    # expire after one week, once rotation is done
    redis.store(key, contents, ex=604800)
    return contents

def clear_redis(clear_files: bool = False) -> None:
    redis.clear(*redis.keys('magic:rotation:summary:*'))
    if clear_files:
        redis.clear(*redis.keys('magic:rotation:file:*'))
